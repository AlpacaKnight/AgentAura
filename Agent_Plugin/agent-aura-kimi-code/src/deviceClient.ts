...
export class RingLightClient {
    ...
    async sendMessage(
        ...
        if (!result.ok && result.statusCode === 404) {
            const reRegistered = await this.registerPetDesktop(runtime.lastState || 'init', runtime.heartbeatIntervalMs || DEFAULT_PETDESKTOP_HEARTBEAT_MS, false, context);
            if (!reRegistered) {
                return false;
            }
        ...
        const latest = loadRuntimeState();
        saveRuntimeState({ ...latest, lastActivityAt: now });
        return true;
    }
    ...
    private httpHeaders(context?: SendContext): Record<string, string> {
        ...
        const headers: Record<string, string> = {
            'x-agentaura-client': 'kimi',
            'x-agentaura-instance': identity.instanceId,
            'x-agentaura-name': 'Kimi Code',
        };
