...
import { clearRuntimeState, configPath, disabledPath, initConfig, isDisabled, kimiHooksPath, loadConfig, loadRuntimeState, saveConfig, setDisabled } from './config';
import { discoverDevices } from './discovery';
import { RingLightClient } from './deviceClient';
import { KIMI_EVENT_TO_AGENT_STATE, KIMI_HOOK_EVENTS, runKimiHook } from './hooks';
import { installKimiHooks, printKimiHooks, uninstallKimiHooks } from './installHooks';
...
async function main(): Promise<void> {
    ...
    switch (command) {
        ...
        case 'doctor':
            ...
            break;
        case 'install-hooks':
            printInstallResult(installKimiHooks());
            break;
        case 'uninstall-hooks':
            console.log(`Removed AgentAura Kimi Code hooks from: ${uninstallKimiHooks()}`);
            break;
        case 'hooks':
            hooksCommand(args);
            break;
        ...
        case 'off':
            setDisabled(true);
            console.log(`AgentAura Kimi Code disabled: ${disabledPath()}`);
            break;
        case 'enable':
        case 'on':
            setDisabled(false);
            console.log('AgentAura Kimi Code enabled');
            break;
        case 'reset-cache':
            clearRuntimeState();
            console.log('Cleared AgentAura Kimi Code runtime state.');
            break;
        case 'help':
        case '--help':
        case '-h':
...
async function configCommand(args: string[]): Promise<void> {
    ...
    throw new Error('Usage: agent-aura-kimi-code config get|path|init|set ...');
}
...
async function status(args: string[]): Promise<void> {
    ...
    const result: Record<string, unknown> = {
        ...
        kimiHooksPath: kimiHooksPath(),
        config,
        client: client.describe(),
        runtime: loadRuntimeState(),
        hookMapping: KIMI_EVENT_TO_AGENT_STATE,
    };
...
function printInstallResult(filePath: string): void {
    console.log(`Installed Kimi Code hooks: ${filePath}`);
    console.log(`Events: ${KIMI_HOOK_EVENTS.join(', ')}`);
}
