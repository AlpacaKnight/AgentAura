...
$zipPath = Join-Path $OutDir "agent-aura-kimi-code-$($manifest.version).zip"
if (Test-Path $zipPath) {
    Remove-Item $zipPath -Force
}

$staging = Join-Path $env:TEMP "agent-aura-kimi-code-pack-$(Get-Random)"
try {
...
$tgz = Get-ChildItem -LiteralPath $OutDir -Filter 'agent-aura-kimi-code-*.tgz' | Sort-Object LastWriteTime -Descending | Select-Object -First 1
Write-Host ''
...
Write-Host '  agent-aura-kimi-code configure --discover'
Write-Host '  agent-aura-kimi-code install-hooks'