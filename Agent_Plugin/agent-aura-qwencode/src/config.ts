...
export function qwenDir(): string {
    return process.env.QWEN_CODE_HOME || process.env.QWEN_HOME || path.join(os.homedir(), '.qwen');
}

export function configPath(): string {
    return process.env.AGENTAURA_QWENCODE_CONFIG || path.join(qwenDir(), 'agent-aura-qwencode.json');
}
...
export function runtimeStatePath(): string {
    return path.join(qwenDir(), 'agent-aura-qwencode-state.json');
}

export function disabledPath(): string {
    return path.join(qwenDir(), 'agent-aura-qwencode.disabled');
}

export function qwenSettingsPath(): string {
    return process.env.QWEN_CODE_SETTINGS || process.env.QWEN_SETTINGS_PATH || path.join(qwenDir(), 'settings.json');
}
...
export function readHooksText(): string {
    const filePath = qwenSettingsPath();
    try {
        ...
        throw new Error(`Cannot read Qwen Code settings file ${filePath}: ${message}`);
    }
}

export function writeHooksText(text: string): void {
    fs.mkdirSync(path.dirname(qwenSettingsPath()), { recursive: true });
    fs.writeFileSync(qwenSettingsPath(), text, 'utf8');
}
...
function applyEnvironmentOverrides(config: AgentAuraConfig): void {
    const enabled = readEnv('AGENTAURA_QWENCODE_ENABLED', 'AGENTAURA_ENABLED');
    if (enabled !== undefined) { config.enabled = parseBoolean(enabled); }

    const transport = readEnv('AGENTAURA_QWENCODE_TRANSPORT', 'AGENTAURA_TRANSPORT');
    if (transport !== undefined && isTransportName(transport.toLowerCase())) {
        config.transport = transport.toLowerCase() as TransportName;
    }

    const host = readEnv('AGENTAURA_QWENCODE_HOST', 'AGENTAURA_HOST');
    if (host !== undefined) { config.host = host.trim(); }

    const port = readEnv('AGENTAURA_QWENCODE_PORT', 'AGENTAURA_PORT');
    if (port !== undefined) { config.port = Number(port); }

    const serialPort = readEnv('AGENTAURA_QWENCODE_SERIAL_PORT', 'AGENTAURA_SERIAL_PORT');
    if (serialPort !== undefined) { config.serialPort = serialPort.trim(); }

    const baud = readEnv('AGENTAURA_QWENCODE_BAUD', 'AGENTAURA_BAUD');
    if (baud !== undefined) { config.baud = Number(baud); }

    const debounceMs = readEnv('AGENTAURA_QWENCODE_DEBOUNCE_MS', 'AGENTAURA_DEBOUNCE_MS');
    if (debounceMs !== undefined) { config.debounceMs = Number(debounceMs); }

    const cooldownMs = readEnv('AGENTAURA_QWENCODE_COOLDOWN_MS', 'AGENTAURA_COOLDOWN_MS');
    if (cooldownMs !== undefined) { config.cooldownMs = Number(cooldownMs); }

    const timeoutMs = readEnv('AGENTAURA_QWENCODE_TIMEOUT_MS', 'AGENTAURA_TIMEOUT_MS');
    if (timeoutMs !== undefined) { config.timeoutMs = Number(timeoutMs); }

    const autoDiscover = readEnv('AGENTAURA_QWENCODE_AUTO_DISCOVER', 'AGENTAURA_AUTO_DISCOVER');
    if (autoDiscover !== undefined) { config.autoDiscover = parseBoolean(autoDiscover); }

    const authToken = readEnv('AGENTAURA_QWENCODE_AUTH_TOKEN', 'AGENTAURA_AUTH_TOKEN');
