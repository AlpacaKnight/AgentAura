...
| Copilot 状态 | 硬件状态 | 灯效 |
|---|---|---|
| 推理 / 生成回复 / 执行工具 | `running` | 🟢 绿色呼吸 |
| 等待用户审批（确认工具调用） | `waiting` | 🟡 黄色闪烁 |
| 回复完成 | `idle` | 🔵 蓝色呼吸 |
| 插件启动 | `init` | 🌈 彩虹 (3秒后自动关灯) |
| 插件关闭 | `offline` | ⚫ 关灯 |

**工作原理**：Copilot 会把每一轮对话的生命周期事件（`user.message`、`assistant.turn_start`、`tool.execution_start`、`assistant.turn_end` 等）写入本地 transcript 日志文件。插件实时监听该文件的追加写入，解析事件并切换灯光状态：

- 收到用户消息 / 开始推理 / 工具执行中 → 🟢 绿色
- 工具请求后 2 秒内未执行（说明在等你点确认）→ 🟡 黄色闪烁
- 整轮回复结束且 6 秒内无新活动 → 🔵 蓝色

> 说明：长时间推理或长命令执行（数十秒至数分钟）期间灯光会稳定保持绿色，不会误判为空闲。
