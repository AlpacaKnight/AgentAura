...
<section class="panel"><h2>桌宠状态（11 种动画）</h2><div class="actions" id="states"><button data-c="pet state idle">待机</button><button data-c="pet state running_right">向右跑</button><button data-c="pet state running_left">向左跑</button><button data-c="pet state speaking">挥手 / 说话</button><button data-c="pet state jumping">跳跃</button><button data-c="pet state error">失败</button><button data-c="pet state waiting">等待</button><button data-c="pet state running">工作中</button><button data-c="pet state thinking">审阅 / 思考</button><button data-c="pet state offline">抬头查看</button><button data-c="pet state sleep">低头休眠</button></div><div class="row"><label for="speech">气泡文字</label><input id="speech" type="text" maxlength="100" placeholder="显示在桌宠上方的文字"><button id="speak">发送</button></div></section>
