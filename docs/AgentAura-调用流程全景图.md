...
flowchart LR
  ...
  subgraph SOURCES["Agent / IDE / App 事件源"]
    direction TB
    CODEX["Codex CLI<br/>命令型 hooks<br/>SessionStart / PreToolUse / PermissionRequest / PostToolUse"]:::host
    CLAUDE["Claude Code<br/>Marketplace 插件 + hooks.json<br/>会话 / 工具 / 审批 / 停止事件"]:::host
    KIMI["Kimi Code CLI<br/>config.toml [[hooks]]<br/>SessionStart / PreToolUse / PermissionRequest / Stop 等"]:::host
    ZCODE["ZCode<br/>插件内 hooks.json<br/>SessionStart / PreToolUse / PermissionRequest / Stop 等"]:::host
  ...
  subgraph PLUGINS["AgentAura 插件层：捕获 → 映射 → 去抖/冷却 → 气泡摘要"]
    direction TB
    PCODEX["agent-aura-codex<br/>hooks.ts → DeviceClient<br/>buildCodexMessage → sendMessage<br/>另含 idle-fallback 定时器"]:::plugin
    PCLAUDE["agent-aura-claude<br/>hooks.ts → RingLightClient<br/>buildClaudeMessage → sendMessage<br/>另有 /agent-aura-claude:aura"]:::plugin
    PKIMI["agent-aura-kimi-code<br/>hooks.ts → DeviceClient<br/>buildKimiMessage → sendMessage"]:::plugin
    PZCODE["agent-aura-zcode<br/>hooks → RingLightClient<br/>buildZcodeMessage → sendMessage<br/>HTTP/UDP/Serial 桥接"]:::plugin
    PQWENCODE["agent-aura-qwencode<br/>hooks.ts → DeviceClient<br/>buildQwenMessage → sendMessage<br/>另含 Qwen Code 扩展安装"]:::plugin
  ...
  subgraph DESKTOP["PetDesktop（Tauri 2 + Rust + React）"]
    ...
    V1["多 Agent API<br/>register / heartbeat(10s) / state / message<br/>list / selection / delete<br/>HTTP /api/v1/agents/*"]:::core
    ARB["AppCore 状态仲裁<br/>锁定来源优先；否则按状态优先级 + 最近活跃<br/>error > waiting > upgrade > busy > running > init > idle > offline"]:::core
    MSG["气泡消息队列<br/>submit_message → per-agent VecDeque(20)<br/>去重 / 优先级替换 / TTL 过期"]:::core
    SNAP["snapshot-changed 事件<br/>AppSnapshot（含 petMessages）"]:::core
    REACT["React 管理 App<br/>Agents / Pets / Hardware / Settings / Logs<br/>Tauri invoke + event listen"]:::output
    PET["透明桌宠窗口<br/>effectiveState → spritesheet 动画行<br/>PetBubble 气泡（状态模板 + 事件摘要）<br/>闲逛 / 置顶 / 穿透"]:::output
    BRIDGE["Hardware Worker 桥接<br/>mpsc 队列；状态变化或白名单命令<br/>HTTP / UDP / Serial"]:::core
  end

  HTTP -->|"兼容状态 / 命令"| DSERV
  UDP -->|"兼容状态 / 命令"| DSERV
  HTTP -. "注册 / 心跳 / 状态 / 气泡消息" .-> V1
  DSERV --> ARB
  V1 --> ARB
  V1 --> MSG
  ARB --> SNAP
  ...
  NOTE["两条有效主链路<br/>A. Agent → 插件 → ESP32（直连）<br/>B. Agent → 插件 → PetDesktop 仲裁/桌宠/气泡 → ESP32（桥接）"]:::optional
  NOTE -.-> CANON
```
...
| Agent 插件（气泡消息） | PetDesktop | HTTP POST `/api/v1/agents/{id}/message` | `kind` / `text` / `priority` / `ttlMs` 气泡消息摘要 |
